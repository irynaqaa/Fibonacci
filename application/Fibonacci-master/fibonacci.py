...
FIRST_POINTS  = int(100)
PREDICT_POINT = int(500)

# Introduction to user
print("Welcome!")

###############################
### (1.)  Graph first 100 (FIRST_POINTS) Fibonacci numbers
###############################
y = fb.fibList(FIRST_POINTS)
x = list(range(1,FIRST_POINTS+1))  # Adjusts for index starting at 0
print("Would you like to see a graph of the first", FIRST_POINTS, "Fibonacci numbers? ")
show_it = strtobool(input("  [Figure 1]  Y/N: "))
if show_it:
    plt.figure(1)
    plt.plot(x, y)
    plt.xlabel('n^th Fibonacci number')
    plt.ylabel('Fibonacci number')
    plt.title('First ' + str(FIRST_POINTS) + ' Fibonacci numbers')
    #plt.yscale('log')  # Uncomment to see log graph
    pylab.show(block=False)
print("From Figure 1, we can see that the Fibonacci numbers grow exponentially.")
...
# Find 500th (PREDICT_POINT) Fibonacci number as well
predictList = fb.fibList(PREDICT_POINT)
...
fitlog500prediction = fitLogPrediction(PREDICT_POINT,popt[0],popt[1])
actual500num = predictList[PREDICT_POINT-1]
# Find error bars for extrapolated predictions
logerror = math.sqrt(mean_squared_error(log_y, fit_y))*math.sqrt(FIRST_POINTS)
lower500bound = math.exp(fitlog500prediction-logerror)
upper500bound = math.exp(fitlog500prediction+logerror)
print("Fitting a line to the data, the best fit has slope:", popt[0], "and y-intercept", popt[1])
...
print("  which should yield the golden ratio.")
print("Compare the fit's prediction: ", math.exp(popt[0]))
print("     to the golden ratio phi: ", fb.phi)
print("So after using only", FIRST_POINTS, "Fibonacci numbers, the fit behavior differs from")
print("     the theoretical limit by", 100*(1- math.exp(popt[0])/fb.phi), "%")

print("Would you like to see a graph of the line we fit to the \
first", FIRST_POINTS,
"Fibonacci numbers, on a log scale? ")
show_it = strtobool(input("  [Figure 2]  Y/N: "))
if show_it:
    plt.figure(2)
    plt.subplot(211)
    ...
    plt.subplot(212)
    plt.plot(log_x + [PREDICT_POINT], log_y + [math.log(actual500num)], "bs")  # Log values in blue squares
    plt.plot(log_x + [PREDICT_POINT], fit_y + [fitlog500prediction], 'r-')  # Fit in red dashes
    plt.xlabel('n^th Fibonacci number')
    plt.ylabel('Log of Fibonacci number')
    plt.title('Same graph, extrapolated to predict 500th Fibonacci number')
    pylab.show(block=False)
print("From Figure 2, we can see that the fit seems to match the first", int(FIRST_POINTS),"Fibonacci numbers well,")
print("and extrapolating the fit to predict the", int(PREDICT_POINT),"th Fibonacci number seems pretty good also.")
print("In fact, the fit predicts the{0:4d}th Fibonacci number to be {1:12E},".format(int(PREDICT_POINT), math.exp(fitlog500prediction)))
print("  within [{0:12E}, {1:12E}]. \
The actual value, {2:12E}, is within these bounds.".format( lower500bound, upper500bound, actual500num))
print("Now let us investigate how much the fit is off by.")
...
print("We have calculated the fit's prediction of the first", FIRST_POINTS, "Fibonacci numbers,")
print("to compare to the actual Fibonacci numbers.")

## Differences from actual Fibonacci numbers
print("Would you like to see graphs of the differences, and percent differences,
between the fit's prediction and the actual Fibonacci number?")
show_it = strtobool(input("  [Figure 3]  Y/N: "))
if show_it:
    plt.figure(3)
    plt.subplot(211)
...
print("Binet's formula is meant for large n (where the ratio of Fibonacci numbers is ")
...
print("Would you like to see the same graphs as before (differences and percent differences),
but using Binet's formula to predict the n^th Fibonacci number instead of the fit we made?")
show_it = strtobool(input("  [Figure 4]  Y/N: "))
if show_it:
    plt.figure(4)
    plt.subplot(211)
...
print("Our fit is limited by only being able to look at the first", FIRST_POINTS, "Fibonacci numbers.")
...
print("We now move on to part 5, simulating some data collection process, which")
...
## Plot of noise
print("Would you like to see a plot of the noise we added to the Fibonacci numbers?")
show_it = strtobool(input("  [Figure 5]  Y/N: "))
if show_it:
    plt.figure(5)
...
## Clean-up noise
print("We clean-up the data, by nudging the noisy value to the nearest Fibonacci number.")
...
print("When cleaning up the data, we were wrong",len(wrongPrediction),"out of", NUMBEROFDATAPOINTS, "times.")
print("Would you like to see a plot of how often we were wrong in cleaning up the data?")
show_it = strtobool(input("  [Figure 6]  Y/N: "))
if show_it:
    plt.figure(6)
...
show_it = strtobool(input("  [Table 1]  Y/N: "))
if show_it:
